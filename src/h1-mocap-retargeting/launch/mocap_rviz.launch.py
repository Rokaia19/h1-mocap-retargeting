...
def generate_launch_description():
    ...
    args = [
        ...
        DeclareLaunchArgument('fps', default_value='60.0',
            description='Mocap capture rate in Hz -- confirmed from the .c3d files ROTATION:RATE'),
        ...
        DeclareLaunchArgument('use_root_motion', default_value='true',
            description='Solve+broadcast a world->pelvis TF (foot-locked IK) instead of a fixed pelvis'),
        DeclareLaunchArgument('use_mink', default_value='true',
            description='Use the Mink QP solver for pelvis IK (requires pip install mujoco mink); '
                        'false falls back to the original Levenberg-Marquardt solver'),
    ...
    retarget_params = {
        ...
        'use_root_motion': LaunchConfiguration('use_root_motion'),
        'use_mink': LaunchConfiguration('use_mink'),
