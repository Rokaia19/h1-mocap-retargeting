...
This is a standalone ROS2 (ament_python) package -- clone it directly into the
`src/` of any existing colcon workspace. The RViz launch (`mocap_rviz.launch.py`)
loads the H1 URDF from `ros_gz_h1_description` (see `package.xml`'s `<depend>`),
so that package must also be present in the same workspace; `retarget_mink` and
`use_mink:=true` (see below) don't need it since they use this package's own
bundled MuJoCo model instead.
...
`convert_urdf_to_mjcf.py` is a one-time/offline regeneration tool, not needed
at runtime -- its `../../ros_gz_h1_description` source path assumes this
package still sits under a workspace `src/` next to `ros_gz_h1_description`
(the layout this repo had before it was split out into its own standalone
package). The `model/*.xml` files it produces are already committed, so
nothing needs re-running unless the source URDF changes; if so, check out
this script next to a `ros_gz_h1_description` checkout, or point
`SRC_URDF`/`SRC_MESHES` at wherever that URDF lives.
